    ...
    */
    ...
    SVM.prototype = {
        ...
        train: function(data, labels, options) {
            ...
            let input_f = options.input_f || null;
            if(input_f){
                let app = copyArray(this.data);
                for(let i=0;i<this.data.length;i++){
                    app[i] = input_f(this.data[i]);
                }
                this.data = app;
            }
            //***************
            /*
            let x2 = options.x2 || false; //decide if use x^2
            let y2 = options.y2 || false; //decide if use y^2
            let x2y2 = options.x2y2 || false; //decide if use x^2+y^2
            let x2_y2 = options.x2_y2 || false; //decide if use x^2-y^2
            let xy = options.xy || false; //decide if use xy
            let sinx = options.sinx || false; //decide if use sin(x)
            let cosx = options.cosx || false; //decide if use cos(x)

            if(x2){
                let app = zeros(this.data.length);
                for(let i=0;i<this.data.length;i++){
                    app[i] = [this.data[i][0],this.data[i][1],Math.pow(this.data[i][0],2)];
                }
                this.data = app;
            }
            if(y2){
                let app = zeros(this.data.length);
                for(let i=0;i<this.data.length;i++){
                    app[i] = [this.data[i][0],this.data[i][1],Math.pow(this.data[i][1],2)];
                }
                this.data = app;
            }
            if(x2y2){
                let app = zeros(this.data.length);
                for(let i=0;i<this.data.length;i++){
                    app[i] = [this.data[i][0],this.data[i][1],(Math.pow(this.data[i][0],2)+Math.pow(this.data[i][1],2))];
                }
                this.data = app;
            }
            if(x2_y2){
            ...
            if(xy){
                let app = zeros(this.data.length);
                for(let i=0;i<this.data.length;i++){
                    app[i] = [this.data[i][0],this.data[i][1],this.data[i][0]*this.data[i][1]];
                }
                this.data = app;
            }
            ...
            if(sinx){
                let app = zeros(this.data.length);
                for(let i=0;i<this.data.length;i++){
                    app[i] = [this.data[i][0],this.data[i][1],Math.sin(this.data[i][0])];
                }
                this.data = app;
            }
            if(cosx){
                let app = zeros(this.data.length);
                for(let i=0;i<this.data.length;i++){
                    app[i] = [this.data[i][0],this.data[i][1],Math.cos(this.data[i][0])];
                }
                this.data = app;
            }
            */
        ...
        kernelFormula: function(i){
            let formula = "";
            formula+="kernel(input,data["+i+"])";
            return formula;
        },
        ...
        getFormula: function(){
            ...
            for(let i=0;i<this.data.length;i++) { //for every data entry (N times)
                formula += this.alpha[i].toPrecision(5)+"*"+this.labels[i]+"*"+this.kernelFormula(i)+"+"; //sum of all these product, including kernel evaluation with
            }
            return formula+" "+this.b;
        },

        getFormulaLinear: function(wb){
          let formula = "";
          for(let i=0;i<wb.w.length;i++){
              formula += wb.w[i].toPrecision(6)+"*"+"input["+i+"]+";
          }
          return formula+""+wb.b.toPrecision(6);
        },
         ...
         */
        SMO: function () {
            console.info("🐻 SMO: "+this.data.length);
            this.update();
            //let statistics = {};
            if(this.N === 0) return; // statistics;
            let numChanged  = 0;
            let examineAll = 1;
            let iter = 0;
    ...
    //              console.info(i+" -> tn");
        ...
        */
        ...
        SSCA: function (D,options) {
            ...
            let alpha = copyArray(this.alpha);
            let labels = copyArray(this.labels);
            let data = copyArray(this.data);
