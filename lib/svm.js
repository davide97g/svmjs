  ...
  */
  ...
  SVM.prototype = {
    ...
    train: function(data, labels, options) {
      ...
      if(SSCA){
        console.info("data before ssca: "+data.length);
        this.SSCA(UB,options);
        console.info("data after ssca: "+data.length);
        //retrain SVM
        stats = this.SMO();
      }
...
//              console.info(i+" -> tn");
    ...
    SSCA: function (D,options) {
      ...
      let alpha = this.copyArray(this.alpha);
      let labels = this.copyArray(this.labels);
      let data = this.copyArray(this.data);
