  ...
  */
  ...
  SVM.prototype = {
    ...
    train: function(data, labels, options) {
      ...
      this.optimized = false; //for the SSCA
      ...
      this.D = data[0].length; let D = this.D;
      this.alpha = zeros(N);
      this.b = 0.0;
    ...
    marginOne: function(inst) {
      ...
      if(this.usew_) { //only with linear kernel
        ...
        for(let i=0;i<this.N;i++) { //for every data entry (N times)
    ...
    objFunction: function(a,i){
      ...
      for(let j=0;j<alpha.length;j++){
        for(let k=0;k<alpha.length;k++){
          s+=labels[j]*labels[k]*this.kernelResult(j,k)*alpha[j]*alpha[k];
        }
      }
      return s;
    },
    ...
    takeStep: function(i1,i2,i,j){
      ...
      if(eta > 0){
        ...
        let Lobj = this.objFunction(L,j);
        let Hobj = this.objFunction(H,j);
    ...
    examineExample: function(i2, i){
      ...
      let Ei= this.marginOne(i2) - y2;
      let r2 = Ei*y2;
      let i1;
      if( (r2 < -tol && alph2 < C) || (r2 > tol && alph2 > 0) ) {
        if(this.countNonBounded() > 1){ //number of non-zero & non-C alpha > 1
        ...
        let rand = randi(0,this.N);
        let counter=0;
        for(let j=rand;counter<this.data.length;j++){ //loop over all non-zero and non-C alpha, starting at a random point
          if(j === this.data.length){
            j=-1;
            continue; //skip this cycle
          }
          if(this.alpha[j]!==0 && this.alpha[j]!== C){ //check
            i1 = this.data[j];
    ...
    SMO: function () {
      ...
      while(numChanged > 0 || examineAll){
        numChanged = 0;
        if(examineAll){
          for (let i=0;i<this.data.length;i++){ //loop over all training examples
            numChanged += this.examineExample(this.data[i],i);
          }
        }
        else {
          for (let i=0;i<this.data.length;i++){ //loop over examples
            if(this.alpha[i] !== this.C && this.alpha[i] !== 0) //where alpha is not 0 & not C
              numChanged += this.examineExample(this.data[i],i);
          }
        }
        if(examineAll === 1)
      ...
      /*
      //time for SCA
      let d = 0.5; //need to be tested
      if(!this.optimized)
        this.SSCA(d);
      */
    },
    
    ruleA: function(labels,i){
      labels[i] = -labels[i];
    },
    
    ruleB: function(data,labels,alpha,i){
      //elimino da tutti gli array l'elemento i
      data = data.splice(i,1);
      labels = labels.splice(i,1);
      alpha = labels.splice(i,1);
    },
    ...
    SSCA: function (D) {
      ...
      let labels = this.labels;
      let value;
      for(let i=0; i<data.length;i++){ //per ogni dato
        value = this.marginOne(data[i])*labels;
        if(value<0){ //misclassified
          this.ruleA(labels,i); //flip label
          value = this.marginOne(data[i])*labels;
          if(value>=0) continue; //se è positivo non è più misclassified
          this.ruleB(data,labels,alpha,i); //lo rimuovo
        }
        if(value < D){ //misclassified under a threshold D
          this.ruleB(data,labels,alpha,i); //rimuovo lo stesso
        }
      }
      //salvo i cambiamenti
      //training set modificato
      this.data = data;
      this.labels = labels;
      this.alpha = alpha;
      ...
      this.optimized = true;
      //train again
      this.train(data,labels,this.options);
