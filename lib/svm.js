  ...
  SVM.prototype = {
    ...
    train: function(data, labels, options) {
      ...
      if(SSCA){
        ...
        let stats2 = this.SMO(); //retrain SVM
        stats.iters = (stats2.iters+stats.iters);
        stats.data = stats2.data;
        stats.labels = stats2.labels;
      }
    ...
    objFunction: function(a,i){
      ...
      for(let j=0;j<limit;j++){
        alpha.push(this.alpha[i]);
        labels.push(this.labels[i]);
      }*/
      let limit = this.alpha.length;
      this.alpha[i] = a;
      for(let j=0;j<limit;j++){ //N times
        for(let k=0;k<limit;k++){ //N times
          s+=this.labels[j]*this.labels[k]*this.kernelResult(j,k)*this.alpha[j]*this.alpha[k];
        }
      }
      s = s/2;
      for(let j=0;j<limit;j++){
        s+=this.alpha[j]*(this.b*this.labels[j]-1);
      }
      return s;
    },
    ...
    takeStep: function(i1,i2,i,j){
      ...
      if(eta > 0){
        ...
        let value = this.alpha[j]; //mi salvo il valore
    ...
    examineExample: function(i2, i){
      ...
      if( (r2 < -tol && alph2 < C) || (r2 > tol && alph2 > 0) ) {
        //console.info("tolerance passed");
        if(this.countNonBounded() > 1){ //number of non-zero & non-C alpha > 1
          //console.info("more than 1 non at bounds");
        ...
        let counter=0;
        let rand = randi(0,limit);
        for(let j=rand;counter<limit;j++){ //loop over all non-zero and non-C alpha, starting at a random point
          if(j === limit){
            j=-1;
            continue; //skip this cycle
          }
          if(!this.isAtBounds(this.alpha[i])){ //check
            //console.info("not at the bounds: "+this.alpha[i]);
            i1 = this.data[j];
        ...
        counter = 0;
        rand = randi(0,limit);
        for(let j=rand;counter<limit;j++){ //loop over all possibile i1, starting at a random point
          if(j === limit){
            j=-1;
            continue; //skip this cycle
          }
    ...
    assignProbabilities: function(max){
        ...
        for(let i=0;i<this.data.length;i++){
          ...
          inst.score = this.logisticValue( value,1).toPrecision(2);
          rich_data[i] = inst;
        }
        //sorting for max
    ...
    SSCA: function (D,options) {
      ...
      for(let i=0; i<this.data.length;i++){ //per ogni dato
        ...
        if(value<0){ //misclassified
          ...
          value = this.marginOne(this.data[i])*this.labels[i];
          if(value>=0) continue; //se è non negativo non è più misclassified
          this.ruleB(i); //lo rimuovo
        }
        //SSCA
        if(value < D){ //misclassified under a threshold D
          this.ruleB(i); //rimuovo lo stesso
        }
      }
