      ...
      for(var i=0;i<this.out_depth;i++) {
        ...
        for(var d=0;d<this.num_inputs;d++) {
          if(formulas.length === 0)
            formulaNet += "x["+d+"]*" + wi[d].toPrecision(4);
          else formulaNet += formulas[d]+ "*" + wi[d].toPrecision(4);
        }
        if(this.biases.w[i]>0)
          formulaNet+="+"+this.biases.w[i].toPrecision(4)+" ";
        else formulaNet+=this.biases.w[i].toPrecision(4)+" ";
        formule.push(formulaNet);
      }
      return formule;
    },
    // forward: function (input,out_depth) {
    //   let V = new Vol(1,1,out_depth,0.0);
    //   V.w = input;
    //   this.in_act = V;
    //   var A = new Vol(1, 1, out_depth, 0.0);
    //   var Vw = input;
    //   for(var i=0;i<out_depth;i++) {
    //     var a = 0.0;
    //     var wi = this.filters[i].w;
    ...
    getFormula: function(){
      ...
      for(var i=1;i<this.layers.length;i++) {
        if(this.layers[i].layer_type==="fc") {
          formulas = this.layers[i].getFormula(formulas);
        }
        else if(this.layers[i].layer_type!=="input" && this.layers[i].layer_type!=="softmax" ){
          //layer della funzione tanh
          formulas = this.layers[i].getFormula(formulas);
        }
      }
      for(let i=0;i<formulas.length;i++){
        formulaNet+=formulas[i];
      }
      return formulaNet;
    },
