...
function draw(){
    ...
    for(var x=0.0, cx=0; x<=WIDTH; x+= density, cx++) {
        for(var y=0.0, cy=0; y<=HEIGHT; y+= density, cy++) {
            ...
            let input = netx.w;
            var a = net.forward(netx, false);
            //var a = net.predictOne(input, net.out_depth);
...
function printWeights(){
    ...
    for(let i=0;i<layers.length;i++){
        ...
        if(layer_type==="fc"){
            weights.push(layers[i].biases.w);
        }
    }
    ...
    for(let i=0;i<weights.length;i++){
        ...
        for(let j=0;j<weights[i].length;j++){
            weights_html += "<li>"+weights[i]+"</li>";
        }
        weights_html += "</ol></li>";
    }
    weights_html += "</ol>";
    document.getElementById("weights").innerHTML=weights_html;
}